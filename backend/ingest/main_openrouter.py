...
from llm_openrouter import texto_para_json_or as texto_para_json
...
def rodar_pipeline(pdf_path):
    ...
    print(f"Processando {len(doc)} páginas com OpenRouter (Matriz de Chaves e Modelos)...")
    ...
    for i, pagina in enumerate(doc):
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Extraídos {len(texto_pagina)} caracteres. Iniciando extração inteligente...")
        
        try:
            dados_json = texto_para_json(texto_pagina)
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Parsing finalizado com sucesso! Salvando no banco...")
            
            if dados_json:
                questoes = validar(dados_json)
                salvar(questoes)
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Página {i+1} salva: ({len(questoes)} questões extraídas).")
            else:
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Página {i+1} não retornou questões válidas.")
        except Exception as e:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Erro severo na página {i+1}: {e}")
...
if __name__ == "__main__":
    ...
    if not pdf_files:
        ...
        for pdf in pdf_files:
            print(f"\n========== INICIANDO INGESTÃO OPENROUTER: {os.path.basename(pdf)} ==========")

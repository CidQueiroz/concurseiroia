...
from llm_groq import texto_para_json_groq as texto_para_json
...
def rodar_pipeline(pdf_path):
    ...
    print(f"Processando {len(doc)} páginas com GROQ...")
    ...
    for i, pagina in enumerate(doc):
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Extraídos {len(texto_pagina)} caracteres. Enviando para a nuvem (Groq)...")
        
        try:
            # O modelo processa um bloco pequeno de cada vez
            dados_json = texto_para_json(texto_pagina)
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Groq respondeu na velocidade da luz! Realizando parsing...")
...
if __name__ == "__main__":
    ...
    if not pdf_files:
        ...
        for pdf in pdf_files:
            print(f"\n========== INICIANDO PROCESSAMENTO TURBO: {os.path.basename(pdf)} ==========")
